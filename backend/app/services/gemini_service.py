...
async def process_transcript_with_gemini(transcript: str, meeting_title: str, host_name: str, employees: list = None):
    """
    Processes the meeting transcript using the configured Groq model.
    Uses the EXACT logic, parameters, and streaming requested by the user.
    """
    cleaned_transcript = clean_transcript(transcript)
    
    # Format employees list for the prompt
    team_members_str = "No specific team members registered yet."
    if employees:
        team_members_str = "\n".join([f"- {e.name} (Emp ID: {e.emp_id})" for e in employees])
...
- ONLY assign tasks to the people listed above. 
- If a task is for someone NOT in the list above, set "assignee_name" to "Unassigned" and "owner_emp_id" to null.
- In the task "description", mention clearly who was originally mentioned (e.g., "Originally mentioned: John Doe").
...
{{
  ...
  "tasks": [
    {{
      ...
      "assignee_name": "<full name of the person responsible or 'Unassigned'>",
      "owner_emp_id": "<employee ID from list above, or null>",
