...
const API_BASE_URL = import.meta.env.VITE_API_BASE_URL || (typeof window !== 'undefined' && window.location.port === '5173' ? 'http://localhost:8000/api' : '/api');
...
const NewMeeting: React.FC = () => {
  ...
  const handlePushTask = async (taskId: string) => {
    ...
    try {
      ...
      setIsProcessing(true);
      await fetch(`${API_BASE_URL}/meetings/${processedData.id}/tasks/${taskId}/push`, {
        method: 'POST'
      });
      alert('Task pushed to active repository!');
    } catch (err) {
      console.error("Push failed:", err);
      alert('Push failed. Check .env credentials.');
    } finally {
      setIsProcessing(false);
    }
  };
                               ...
                               {editingTask === idx ? (
                                 ...
                                 <div className="flex items-center gap-2">
                                   <button 
                                      ...
                                      className="text-slate-300 hover:text-accent-teal transition-colors"
